...
import {
  ...
  Image as ImageIcon, Video, Navigation, Store,
  Award, Zap, Calendar, Activity, Smartphone
} from "lucide-react";
...
export default function VendorProfileDetail() {
  ...
  const uploadFileToBucket = async (
    ...
    const filePath = `${folder}/${vendor.id}/${fileName}`;
  ...
  const handleVideoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      if (file.size > 50 * 1024 * 1024) {
        alert("Video is too large. Upload under 50MB.");
        return;
      }

      const url = await uploadFileToBucket(file, "videos", "vendor-videos");

      setEditForm({
        ...editForm,
        video_files: [
          ...(editForm.video_files || []),
          { url, title: file.name, type: "upload" },
        ],
      });
  ...
  const removeCertificate = (index: number) => {
    const updated = editForm.certificates.filter((_: any, i: number) => i !== index);
    setEditForm({ ...editForm, certificates: updated });
  };
            ...
            >
              <div className="p-6 bg-yellow-400 text-black flex justify-between items-center">
                ...
                <button
                  ...
                  className="bg-black text-white p-2 rounded-xl hover:scale-105 transition-all"
                >
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="p-8 overflow-y-auto bg-slate-50 grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <div className="space-y-6">
                  ...
                  <div className="pt-2">
                    <InputField
                      ...
                      value={editForm.business_keywords}
                      onChange={(v: any) => setEditForm({ ...editForm, business_keywords: v })}
                    />
                  </div>
                  {/* Sector Checkboxes */}
                  <div className="w-full">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {sectorOptions.map((option) => {
                        const selectedSectors = editForm.sector ? editForm.sector.split(",") : [];
                        ...
                        return (
                          <label
                            ...
                            ${isChecked ? "bg-yellow-100 border-yellow-400" : "bg-white border-slate-200 hover:border-yellow-300"}`}
                          >
                            <input
                              type="checkbox"
                              checked={isChecked}
                              onChange={(e) => {
                                ...
                                if (e.target.checked) updated.push(option.value);
                                else updated = updated.filter(v => v !== option.value);

                                setEditForm({ ...editForm, sector: updated.join(",") });
                              }}
                ...
                <div className="space-y-6">
                  ...
                  <InputField label="Street" value={editForm.street} onChange={(v: any) => setEditForm({ ...editForm, street: v })} />
                ...
                <div className="space-y-8">
                  ...
                  <div className="pt-4 border-t border-slate-200">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {editForm.certificates?.map((img: string, i: number) => (
                        <div key={i} className="relative aspect-square rounded-xl overflow-hidden border group">
                          <img src={img} className="w-full h-full object-cover" />
                          <button
                  ...
                  <div className="pt-4 border-t border-slate-200">
                    <label className="text-[10px] font-black uppercase text-slate-400 flex justify-between items-center mb-3">
                      ...
                      <label className="bg-black text-yellow-400 p-1.5 rounded-lg cursor-pointer">
                        ...
                        <input type="file" hidden onChange={handleVideoUpload} accept="video/*" />
                      </label>
                    </label>
              ...
              <div className="p-6 bg-white border-t flex gap-3">
                <button
                  ...
                  disabled={isSaving}
                  className="flex-1 bg-black text-yellow-400 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-2"
                >
      ...
      <div className="bg-gradient-to-b from-[#FEF3C7] to-[#FFFDF5] pt-16 pb-32 px-6 relative overflow-hidden border-b border-yellow-200">
        ...
        <div className="max-w-7xl mx-auto relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12">
          <div className="flex flex-col lg:flex-row gap-10 items-center lg:items-end flex-1">
            ...
            >
              <div className="w-32 h-32 md:w-48 md:h-48 bg-white rounded-[2.5rem] p-4 md:p-6 shadow-2xl border-2 border-yellow-100 flex items-center justify-center overflow-hidden">
                {vendor.company_logo ? (
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-16 pb-20 relative z-20 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard label="Contact" value={vendor.mobile_number} icon={<Smartphone size={16} />} />
          </div>
          ...
          <div className="bg-white rounded-[2.5rem] p-6 md:p-10 shadow-sm border border-slate-100">
            ...
            <p className="text-slate-600 text-base md:text-lg leading-relaxed font-medium mb-8 whitespace-pre-line">
          ...
          {vendor.video_files && vendor.video_files.length > 0 && (
            <div className="bg-white rounded-[2.5rem] p-6 md:p-10 shadow-sm border border-slate-100">
              <div className="flex items-center gap-3 mb-8">
                <div className="p-3 bg-black rounded-2xl text-yellow-400">
                  <Video size={20} />
                </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-slate-900 rounded-[3rem] p-6 md:p-8 text-white shadow-2xl space-y-8 border-t-[8px] border-yellow-400">
              ...
              <div className="space-y-3">
                ...
                {vendor.websites.map((url: string, idx: number) => (
                  <a
                    ...
                    target="_blank"
                    className="flex items-center gap-3 bg-white/5 p-4 rounded-2xl hover:bg-yellow-400 hover:text-black transition-all group overflow-hidden"
                  >
            ...
            <div className="pt-8 border-t border-white/5">
              ...
              <address className="text-sm font-bold text-white/80 leading-relaxed uppercase">
                {[vendor.flat_no, vendor.building].filter(Boolean).join(", ")}<br />
                {[vendor.street, vendor.area].filter(Boolean).join(", ")}<br />
                <span className="text-yellow-400 block mt-2 font-black text-lg">
                  {vendor.city}, {vendor.state}
                </span>
                <span className="text-white/30 block tracking-widest">
                  {vendor.pincode}
                </span>
              </address>

              <a
                href={`http://maps.google.com/?q=${encodeURIComponent(`${vendor.company_name} ${vendor.city}`)}`}
                target="_blank"
                className="mt-4 inline-flex items-center gap-2 text-[10px] font-black text-yellow-400 uppercase tracking-widest"
              >
            ...
            <a
              ...
              className="flex w-full bg-yellow-400 text-black py-5 rounded-2xl items-center justify-center gap-3 font-black uppercase text-[10px] tracking-widest hover:bg-white transition-all shadow-xl"
            >
          ...
          <div className="bg-white rounded-[2.5rem] p-6 md:p-8 border border-slate-100 shadow-lg space-y-4">
            ...
            <AccountRow label="Verification" value={vendor.status} />
...
function AccountRow({ label, value }: any) {
  return (
